...
int MPEGaudio_getbits (MPEGaudio *self, int bits)
{
    union
    {
        char store[4];
...
// Convert mpeg to raw
// Mpeg headder class
void MPEGaudio_initialize (MPEGaudio *self)
{
  ...
  if( ! initialized )
  {
    for(i=0;i<16;i++) self->hcos_64[i] = (float)
			(1.0/(2.0*cos(MY_PI*double(i*2+1)/64.0)));
    for(i=0;i< 8;i++) self->hcos_32[i] = (float)
			(1.0/(2.0*cos(MY_PI*double(i*2+1)/32.0)));
    for(i=0;i< 4;i++) self->hcos_16[i] = (float)
			(1.0/(2.0*cos(MY_PI*double(i*2+1)/16.0)));
    for(i=0;i< 2;i++) self->hcos_8 [i] = (float)
...
int MPEGaudio_loadheader (MPEGaudio *self)
{
    register int c;
    bool flag;
    int speed;
    ...
    self->protection = c & 1;
    self->layer = 4 - ((c >> 1) & 3);
    self->version = (_mpegversion) ((c >> 3) ^ 1);

    c = MPEGstream_copy_byte(self->mpeg) >> 1;
    padding = (c & 1);
    c >>= 1;
    self->frequency = (_frequency) (c&3);
    if (self->frequency == 3)
        return false;
    c >>= 2;
  ...
  {
    if(!self->tableindex)
      if(frequency==frequency32000)self->subbandnumber=12; else self->subbandnumber=8;
  ...
  else if(self->mode==joint)self->stereobound=(extendedmode+1)<<2;
  ...
  if(self->layer==1)
  {
    self->framesize=(12000*self->bitrate[self->version][0][self->bitrateindex])/
              self->frequencies[self->version][self->frequency];
    if(self->frequency==frequency44100 && self->padding)self->framesize++;
    self->framesize<<=2;
  }
  else
  {
    self->framesize=(144000*self->bitrate[self->version][self->layer-1][self->bitrateindex])/
      (self->frequencies[self->version][self->frequency]<<self->version);
...
#ifdef DEBUG_AUDIO
  fprintf(stderr, "MPEG %d audio layer %d (%d kbps), at %d Hz %s [%d]\n", self->version+1, self->layer,  self->bitrate[self->version][self->layer-1][self->bitrateindex], self->frequencies[self->version][self->frequency], (self->mode == single) ? "mono" : "stereo", self->framesize);
#endif
  ...
  speed = self->frequencies[self->version][self->frequency];
...
int MPEGaudio_run( MPEGaudio *self, int frames, double *timestamp)
{
