...
void MPEGaudio_subbandsynthesis (REAL *fractionL,REAL *fractionR)
{
  ...
  MPEGaudio_computebuffer(self,fractionL,calcbufferL);
  if(!self->outputstereo) MPEGaudio_generatesingle(self);
  else
  {
    MPEGaudio_computebuffer(self,fractionR,calcbufferR);
